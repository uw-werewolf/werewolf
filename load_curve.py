...
def read_lmp():
    ...
    for i in files:
        n += 1
        print(
            "Processing... " + i + ", " + str(n) + " of " + str(len(files)) + " files"
        )
    ...
    df = pd.melt(
        df, id_vars=["MARKET_DAY", "NODE", "TYPE"], var_name="hour", value_name="LMP"
    )
    ...
    df["MARKET_DAY"] = [
        df.loc[i, "MARKET_DAY"]
        + datetime.timedelta(hours=df.loc[i, "hour"].item())
...
if __name__ == "__main__":
    ...
    gdxout.add_parameter_dc(
        gamsparametername="lmp",
        domain=["hr", "i"],
        toparameter=p,
        desc="LMP price (units: $/MW)",
    )
