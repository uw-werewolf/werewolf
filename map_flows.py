...
def get_bearing(i, j):
    ...
    y = (np.cos(lat_i) * np.sin(lat_j) - (np.sin(lat_i) * np.cos(lat_j) * np.cos(lng_diff)))
...
def make_power_flow_map(geoPts, flows):
    ...
    for j in geoPts:
        circ = folium.Circle([geoPts[j][0], geoPts[j][1]],
                             popup=j,
                             radius=1000,
                             color='orange',
                             fill=True,
    ...
    for i in flows.keys():
        arcs = folium.FeatureGroup('Power Flow in Block {}'.format(i), control=True, show=False)
        for j in flows[i].keys():
            if flows[i][j] != 0:
                ...
                line = folium.PolyLine([(lat_i, lng_i), h, (lat_j, lng_j)],
...
if __name__ == '__main__':
    ...
    for i in scn_lst:
        ...
        flows_p = pd.pivot_table(flows_p, values=['values'], columns=['b'], index=[
            'i', 'j'], fill_value=0).reset_index(drop=False)
