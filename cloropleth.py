...
def solar_capacity_factor(to_csv=False):
    df = pd.read_csv('../../data/processed_data/solar_hourly_capacity_factor.csv', engine='c')
...
def wind_capacity_factor(to_csv=False):
    df = pd.read_csv('../../data/processed_data/wind_hourly_capacity_factor.csv', engine='c')
    df.drop(columns=['Unnamed: 0'], inplace=True)  # drop index_col
    df['timestamp'] = pd.to_datetime(df['timestamp'])  # convert timestamp to type datetime
...
def map_cfs():
    ...
    folium.Choropleth(geo_data=geo_data,
                      ...
                      fill_opacity=0.7,
                      line_opacity=0.2,
                      legend_name='Onshore Wind Capacity Factor (%)').add_to(my_map)
    ...
    folium.Choropleth(geo_data=geo_data,
                      ...
                      fill_opacity=0.7,
                      line_opacity=0.2,
                      legend_name='Offshore Wind Capacity Factor (%)').add_to(my_map)
