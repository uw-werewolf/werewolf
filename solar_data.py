...
def solar_capacity_factor(to_csv=False):
    ...
    cap["total"] = cap[
        ["cost_bin1", "cost_bin2", "cost_bin3", "cost_bin4", "cost_bin5"]
    ].sum(axis=1)

    cap = cap.merge(
        cap[["tech", "reeds.ba", "total"]].groupby("reeds.ba").sum(),
        left_on="reeds.ba",
        right_on="reeds.ba",
    ).copy()
    ...
    pt = pt.merge(
        cap[["tech", "reeds.ba", "weight"]],
        left_on=["tech", "reeds_region"],
        right_on=["tech", "reeds.ba"],
    ).copy()

    pt["cf_weighted"] = pt["value"] * pt["weight"]

    cf = (
        pt[["tech", "reeds_region", "cf_weighted"]]
        .groupby(["reeds_region"])
        .sum()
        .copy()
    )
    ...
    for i in set(pt.reeds_region):
        ...
        average = np.average(
            pt[pt["reeds_region"] == i]["value"].values,
            weights=pt[pt["reeds_region"] == i]["weight"].values,
        )

        cf.loc[i, "std"] = np.sqrt(
            np.average(
                (values - average) ** 2,
                weights=pt[pt["reeds_region"] == i]["weight"].values,
            )
...
if __name__ == "__main__":
    ...
    results = pd.DataFrame(
        data=set(list(zip(cf2.i, cf2.k, cf2.block_hour))),
        columns=["region", "tech", "block_hour"],
    )
    results.sort_values(
        by=["region", "tech", "block_hour"], ignore_index=True, inplace=True
    )
