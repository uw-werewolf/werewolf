...
class WerewolfEnvironment:
    ...
    def __init__(self, gams_sysdir, data_dir, version=None, load=None):
        ...
        print(f"Werewolf data directory given as: {self.data_dir}")
        ...
        self.werewolf_notation = {}
        for k, v in mappings.maps.items():
            for col in v.columns:
                self.werewolf_notation[col] = set(mappings.maps[k][col])
    ...
    def test_notation(self):
        ...
        for k in self.notation_link.keys():
            ...
            for d, nl in self.notation_link[k]:
                ...
                data = set(v[d])
                notation = self.werewolf_notation[nl]
    ...
    def clean(self):
        if self.version == "windc_2_0_1":
            self.werewolf_notation["year"] = set([str(i) for i in range(1997, 2017)])
            ...
            self.werewolf_notation["usda_naics"] = {
                "11111",
                "11112",
                "11113",
                "11114",
            ...
            # create a new notation
            self.werewolf_notation["gsp_industry_id"] = {
                "4",
                "5",
                "7",
                "8",
            ...
            cfs_ma = cfs_ma - {"0"}
            self.werewolf_notation["cfs_ma"] = cfs_ma
            ...
            self.werewolf_notation["cfs_sctg"] = cfs_sctg

            # create a new notation from data
            # drop undisclosed SCTG data
            self.werewolf_notation["cfs_export"] = {"N"}
            ...
            self.werewolf_notation["gsp_componentname"] = gsp

            # create a new notation from data
            self.werewolf_notation["pce_componentname"] = {
                "Total personal consumption expenditures (PCE) by state"
            }

            # create a new notation from data
            self.werewolf_notation["sgf_category"] = set(
                self.data["census_sgf"]["Category"]
            )

            self.werewolf_notation["exim_cnty"] = {"World Total"}
        ...
        if self.version == "windc_2_1":
            self.werewolf_notation["year"] = set([str(i) for i in range(1997, 2018)])
    ...
    def transform(self):
        if self.version == "windc_2_0_1":
            ...
            for i in self.werewolf_notation["year"]:
    ...
    def gdx_data(self):
        if self.version == "windc_2_0_1":
            ...
            return gdx_data.build_data(self.version, self.werewolf_notation, self.data)
