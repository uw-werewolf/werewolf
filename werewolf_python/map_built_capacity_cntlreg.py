...
if __name__ == "__main__":
    ...
    if os.path.isdir(os.path.join(args.output, "built_capacity")) == False:
        os.mkdir(os.path.join(args.output, "built_capacity"))
    ...
    cap["is_cntlreg"] = cap["k"].isin(gdx.to_dict("cntlreg")["elements"])
        ...
        if sum(cap.pct_delta == 0) != len(cap):
            ...
            my_map.save(
                os.path.join(
                    args.output,
                    "built_capacity",
                    f"{a1.replace('/','_').replace(' ','_')}_{a2}_pct_delta_built_cap_cntlreg.html",
                )
            )
        ...
        if sum(cap.L == 0) != len(cap):
            ...
            my_map.save(
                os.path.join(
                    args.output,
                    "built_capacity",
                    f"{a1.replace('/','_').replace(' ','_')}_{a2}_abs_built_cap_cntlreg.html",
                )
            )
